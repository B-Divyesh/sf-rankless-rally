...
The demo opens Practice 01 with a sample rally card and a visible shared-route marker. The product server verifies the seeded route and stores it in its isolated `demo` SQLite namespace with a 24-hour expiry. Demo and public replay records cannot read each other. It is not read from real-game storage. Browser storage uses only these keys:
