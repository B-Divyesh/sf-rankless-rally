...
This product deploys as one Rust container at `https://rankless-rally.sociobot.in`. It serves `dist/` and its replay API from the same origin. The server starts with only `PORT`; it writes SQLite state to `/data/rankless-rally.sqlite3` on the fleet-mounted durable volume and runs as one replica. The product needs no external provider or credential.
